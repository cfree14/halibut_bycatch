...
data <- data_obs %>%
  ...
  mutate(discards_n=n_returned_dead) %>%
  group_by(set_id) %>%
...
g2 <- ggplot(data %>% filter(comm_name %in% top40spp), aes(x=ratio, y=factor(comm_name, levels=top40spp))) +
  ...
  # Labels
  labs(x="Catch ratio\n(dead discards / halibut landings)", y="", tag="B", title=" \n ") +
  # Axis
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_swfsc_observer_discards_dead_ratio_by_species_southern_large.png"),
...
g <- ggplot(data %>% filter(comm_name%in%top20spp), aes(x=year, y=ratio, group=year)) +
  ...
  # Labels
  labs(y="Catch ratio\n(dead discards / halibut landings)", x="Year", title="SWFSC gill net observer data - southern large-mesh fishery") +
  # Axis
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_swfsc_observer_discards_dead_ratio_over_time_southern_large.png"),
...
g <- ggplot(data %>% filter(comm_name%in%top20spp & depth_fa<100), aes(x=depth_fa, y=ratio)) +
  ...
  # Labels
  labs(x="Depth (fathoms)", y="Catch ratio\n(dead discards / halibut landings)", title="SWFSC gill net observer data - southern large-mesh fishery") +
  # Axis
  scale_y_continuous(trans="log10", breaks=c(0.01, 0.1, 1, 10, 100), labels=c("0.01", "0.1", "1", "10", "100")) +
  # Theme
  theme_bw() + theme3
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_swfsc_observer_discards_dead_ratio_by_depth_southern_large.png"),
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_swfsc_observer_discards_dead_ratio_by_date_southern_large.png"),
       width=6.5, height=5.5, units="in", dpi=600)
