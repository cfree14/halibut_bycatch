...
plotdir <- "data/landings_receipts/figures/gillnet"
...
g1 <- ggplot(stats %>% filter(comm_name %in% top40spp), aes(x=psets, y=factor(comm_name, levels=stats$comm_name))) +
  ...
  # Labels
  labs(x="Catch occurrence\n(percent of gill net sets)", y="", tag="A", title="CDFW landing receipts\nsouthern large-mesh fishery") +
  # Axis
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_landing_receipts_bycatch_ratio_by_species_gillnet_southern_large.png"),
...
g <- ggplot(data %>% filter(comm_name%in%top20spp), aes(x=year, y=ratio, group=year)) +
  ...
  # Labels
  labs(y="Catch ratio\n(non-halibut landings / halibut landings)", x="Year", title="CDFW landing receipts - southern large-mesh fishery") +
  # Axis
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_landing_receipts_bycatch_ratio_over_time_gillnet_southern_large.png"),
...
g <- ggplot(data_block_sf %>% filter(comm_name%in%top20spp), aes(fill=ratio_med)) +
  ...
  # Labels
  labs(x="", y="", title="CDFW landing receipts - southern large-mesh fishery") +
  # Legend
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_landing_receipts_bycatch_ratio_over_space_gillnet_southern_large.png"),
...
g <- ggplot(data %>% filter(comm_name%in%top20spp), aes(x=date_dummy, y=ratio)) +
  ...
  # Labels
  labs(x="Day of year", y="Catch ratio\n(non-halibut landings / halibut landings)", title="CDFW landing receipts - southern large-mesh fishery") +
  # Axis
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_landing_receipts_bycatch_ratio_by_date_gillnet_southern_large.png"),
