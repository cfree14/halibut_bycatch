...
g1 <- ggplot(stats %>% filter(comm_name %in% top40spp), aes(x=psets, y=factor(comm_name, levels=stats$comm_name))) +
  ...
  # Labels
  labs(x="Catch occurrence\n(percent of trawl tows)", y="", tag="A", title="CDFW landing receipts - northern trawl fishery") +
  # Axis
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_landings_receipts_bycatch_ratio_by_species_trawl_northern.png"),
...
g <- ggplot(data %>% filter(comm_name%in%top20spp), aes(x=year, y=ratio, group=year)) +
  ...
  # Labels
  labs(y="Catch ratio\n(non-halibut landings / halibut landings)", x="Year", title="CDFW landings receipts - northern trawl fishery") +
  # Axis
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_landings_receipts_bycatch_ratio_over_time_trawl_northern.png"),
...
g <- ggplot(data_block_sf %>% filter(comm_name%in%top20spp), aes(fill=ratio_med)) +
  ...
  # Labels
  labs(x="", y="", title="CDFW landings receipts - northern trawl fishery") +
  # Legend
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_landings_receipts_bycatch_ratio_over_space_trawl_northern.png"),
...
g <- ggplot(data %>% filter(comm_name%in%top20spp), aes(x=date_dummy, y=ratio)) +
  ...
  # Labels
  labs(x="Day of year", y="Catch ratio\n(non-halibut landings / halibut landings)", title="CDFW landings receipts - northern trawl fishery") +
  # Axis
...
# Export plot
ggsave(g, filename=file.path(plotdir, "FigX_landings_receipts_bycatch_ratio_by_date_trawl_northern.png"),
